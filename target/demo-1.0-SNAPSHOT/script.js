...
$(document).on("click", "#getFilmsBtn", function () {
    ...
    $.get("http://localhost:8080/film/getAllFilms", "format=" + format, function (responseText) {
        $("#loading").hide();
        if (format === 'json') {
...
$(document).on("click", "#submitBtn", function () {
    ...
    format = $("#format").val();
    $.get("http://localhost:8080/film/getFilm", params + "&format=" + format, function (responseText) {
        $("#loading").hide();
        if (format === 'json') {
...
jQuery(document).on("click", "#btnSubmit", function () {
        ...
        $.post("http://localhost:8080/film/insert-film" + "?format=" + format, film, function (responseText) {
            $("#loading").hide();
            if (format === 'json') {
...
jQuery(document).on("click", "#submitUpdateBtn", function () {
    ...
    $.ajax({
        url: 'http://localhost:8080/film/update-film' + "?filmId=" + filmId + "&filmTitle=" + filmTitle + "&filmYear=" + filmYear + "&filmDirector=" + filmDirector + "&filmStars=" + filmStars + "&filmReview=" + filmReview + "&format=" + format,
        type: 'PUT',
...
function loadAllFilmsOnDeletePage() {
    ...
    $.get("http://localhost:8080/film/getAllFilms", "format=json", function (responseText) {
        $("#loading").hide();
...
function deleteById(element) {
    ...
    $.ajax({
        url: "http://localhost:8080/film/delete-film?" + "filmId=" + id,
        type: 'DELETE',
        success: function (responseText) {
            showJsonDisplayAllFilmsInfoForDelete(responseText);
            console.log(responseText);
